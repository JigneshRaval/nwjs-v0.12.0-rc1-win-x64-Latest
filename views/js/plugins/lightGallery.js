    ...
    $.fn.lightGallery = function(options) {
        ...
        var setUp = {
            ...
            slideTo: function() {
				...
				console.log("slideTo", $this, this);
				
                if (settings.controls === true && $children.length > 1) {
                    $gallery.append('<div id="lightGallery-action"><a id="lightGallery-prev"></a><a id="lightGallery-next"></a><a id="lightGallery-zoomer"><i class="ion-search"></i></a></div>');
                    $prev = $gallery.find('#lightGallery-prev');
                    $next = $gallery.find('#lightGallery-next');
					
					/* My Changes ========*/
					// Light Gallery Zoom image using http://formstone.it/components/zoomer/
					/*var $lgZoomer = $gallery.find('#lightGallery-zoomer'); 
					
					
                    $lgZoomer.bind('click', function() {
					
						console.log("PPP 1: ", el, plugin);
						
						var index = parseInt($("#lightGallery_counter_current").text());
						index = index - 1;
						if($slide.eq(index).find('.zoomer').length > 0) {
							$slide.eq(index).zoomer("destroy");
							settings.enableDrag = true;
							$this.touch();
						}
						else {
							
							//console.log(index, $slide.eq(index).attr('class'));
							$slide.eq(index).zoomer({
								controls: {
									postion : "top"
								}
							});
						...
						console.log("Zoomer Length :", $slide.eq(index).find('.zoomer').length, settings.enableDrag);
					});
					*/
					/* END :  My Changes ========*/
            ...
            slide: function(index) {
                ...
                $slide.eq(prevIndex).removeClass('current').removeClass('js-zoom');
                $slide.eq(index).addClass('current').addClass('js-zoom');
				
				// My Changes
				$slide.eq(prevIndex).removeClass('js-zoom');
				$slide.eq(index).addClass('js-zoom');
