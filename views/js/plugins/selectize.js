	...
	$.extend(Selectize.prototype, {
		...
		updateOriginalInput: function() {
			...
			if (self.tagType === TAG_SELECT) {
				options = [];
				for (i = 0, n = self.items.length; i < n; i++) {
					//console.log("Selected Value :", escape_html(self.items[i]), self.$dropdown_content.find('.selected').text());
					options.push('<option value="' + escape_html(self.items[i]) + '" selected="selected">' +self.$dropdown_content.find('.selected').text() + '</option>');
				}
